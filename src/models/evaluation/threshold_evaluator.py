...
class ThresholdEvaluator:
    ...
    def _vectorized_profit_calculation(
        ...
        if invest_mask.sum() > 0:
            ...
            current_prices_1d = (
                current_prices.flatten() if current_prices.ndim > 1 else current_prices
            )
    ...
    def _vectorized_threshold_testing(
        self,
        ...
        logger.info(
            f"Sample constraints: min={min_samples}, max={max_samples} (total samples={len(test_confidence)})"
        )
        ...
        for i, threshold in enumerate(thresholds):
            ...
            if investment_decisions.sum() > 0:
                successful_investments = (
                    (filtered_y_true[investment_decisions] > 0)
                    & (
                        filtered_y_true[investment_decisions]
    ...
    def optimize_prediction_threshold(
        ...
        if len(results_df) == 0:
            ...
            return {
                "status": "failed",
                "message": f"No valid thresholds found under the {self.min_samples_percentage * 100}%-{self.max_samples_percentage * 100}% sample constraint",
            }
    ...
    def predict_with_threshold(
        ...
        if not isinstance(model, ModelProtocol):
            raise ValueError(
                "Model must implement ModelProtocol (predict and get_prediction_confidence methods)"
            )
    ...
    def calculate_profit_score(
        ...
        conservative_rate = (
            (correct_conservative_predictions / positive_predictions) * 100
            if positive_predictions > 0
            else 0.0
        )
