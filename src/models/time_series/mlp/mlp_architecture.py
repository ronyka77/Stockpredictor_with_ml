    ...
    def _create_output_layer(self):
        ...
        if self.task == "regression":
            ...
            self.output_layer = nn.Sequential(
                nn.Linear(self.layer_sizes[-1], self.output_size), nn.Sigmoid()
            )
        else:
            raise ValueError(
...
class MLPDataUtils:
    ...
    def scale_data(
        ...
        if tmp.isnull().sum().sum() > 0:
            logger.warning(
                "⚠️ scale_data received data with NaN/Inf - attempting to clean via validate_and_clean_data"
            )
            try:
                tmp = MLPDataUtils.validate_and_clean_data(data)
            except Exception as e:
                raise ValueError(
                    "Input data contains NaN/Inf values and could not be cleaned before scaling"
                ) from e
