...
from src.models.time_series.realmlp.realmlp_predictor import (
    RealMLPPredictor as RealMLPInner,
)
...
class RealMLPPredictor(BasePredictor):
    ...
    def load_model_from_mlflow(self) -> None:
        ...
        if not ok:
            raise RuntimeError(
                f"Failed to load RealMLP model from MLflow run {self.run_id}"
            )
        self.model = inner
