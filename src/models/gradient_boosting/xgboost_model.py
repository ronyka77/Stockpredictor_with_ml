...
class XGBoostModel(BaseModel):
    ...
    def _create_model(
        ...
        **kwargs,
        ...
        config = params.copy()
        config.update({"investment_amount": self.threshold_evaluator.investment_amount})
        ...
        logger.info(
            f"Created new XGBoost model '{model_name}' with parameters: {params}"
        )
    ...
    def fit(
        ...
        logger.info(
            f"Training completed. Best iteration: {best_iteration}, Best score: {best_score:.4f}"
        )

        # Log parameters to MLflow
        self.log_params(params)
        self.log_metrics({"best_iteration": best_iteration, "best_score": best_score})
    ...
    def save_model(
        ...
        logger.info(
            f"✅ XGBoost model saved using universal logging function. Run ID: {run_id}"
        )
        return run_id
    ...
    def load_model(self, run_id: str) -> None:
        ...
        try:
            ...
            for artifact in artifacts:
                logger.info(
                    f"  - {artifact.path} ({'dir' if artifact.is_dir else 'file'})"
                )
            ...
            logger.info(
                f"✅ XGBoost model loaded successfully from MLflow run: {run_id}"
            )

        except Exception as e:
            logger.error(
                f"❌ Error loading XGBoost model from MLflow run {run_id}: {str(e)}"
            )
            raise
    ...
    def _load_metadata_from_run(self, run_info) -> None:
        ...
        try:
            ...
            self.prediction_horizon = int(
                params.get("model_prediction_horizon", self.prediction_horizon)
            )
            self.early_stopping_rounds = int(
                params.get("model_early_stopping_rounds", self.early_stopping_rounds)
            )
            ...
            try:
                ...
                if model_info and model_info.signature:
                    ...
                    if model_info.signature.inputs:
                        ...
                        if feature_names:
                            ...
                            logger.warning(
                                "⚠️ No feature names found in model signature inputs"
                            )
                    else:
                ...
                try:
                        ...
                        if "feature_names.txt" in artifact.path:
                            with tempfile.TemporaryDirectory() as temp_dir:
                                feature_file = client.download_artifacts(
                                    run_info.info.run_id, artifact.path, temp_dir
                                )
                    ...
                    logger.info(
                        f"Could not load feature names from artifacts: {str(artifact_error)}"
                    )
    ...
    def get_prediction_confidence(
        ...
        if method == "leaf_depth":
            ...
            current_prices = (
                X["close"].values if "close" in X.columns else np.ones_like(predictions)
            )
        ...
        def objective(trial):
            ...
            try:
                ...
                if threshold_results["status"] == "success":
                    ...
                    logger.info(
                        f"  Optimized {objective_column}: {optimized_profit_score:.3f}"
                    )
                ...
                if optimized_profit_score > self.best_investment_success_rate:
                    ...
                    if threshold_info["optimal_threshold"] is not None:
                        ...
                        logger.info(
                            f"   Custom accuracy: {threshold_info['custom_accuracy']:.3f}"
                        )

                    self.previous_best = optimized_profit_score
                else:
                    logger.info(
                        f"Trial {trial.number}: Investment Success Rate = {optimized_profit_score:.3f} (Best: {self.best_investment_success_rate:.3f})"
                    )
    ...
    def log_model_to_mlflow(
        ...
        logger.info(
            f"✅ Model logged to MLflow using universal function. Run ID: {run_id}"
        )
        return run_id
...
def main():
    ...
    try:
        ...
        logger.info(
            "4. Making final evaluation with automatically selected best model..."
        )
        ...
        has_threshold_optimization = (
            hasattr(final_model, "best_threshold_info")
            and final_model.best_threshold_info is not None
            and final_model.best_threshold_info.get("optimal_threshold") is not None
        )
        ...
        if has_threshold_optimization:
            ...
            final_investment_success_rate = threshold_performance[
                "investment_success_rate"
            ]
            ...
            threshold_metrics = {
                ...
                "final_improvement_ratio": threshold_performance[
            ...
            final_total_profit = final_model.threshold_evaluator.calculate_profit_score(
                y_test.values, final_predictions, final_current_prices
            )
        ...
        if has_threshold_optimization:
            logger.info(
                f"   Samples Used: {final_samples_kept}/{len(y_test)} (threshold-filtered)"
            )
        else:
            logger.info(
                f"   Samples Used: {final_samples_kept}/{len(y_test)} (all samples)"
            )
        ...
        for i, (_, row) in enumerate(top_features.iterrows()):
            final_metrics[f"feature_importance_{i + 1}_{row['feature']}"] = row[
                "importance"
            ]
