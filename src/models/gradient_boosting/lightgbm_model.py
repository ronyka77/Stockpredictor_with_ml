...
class LightGBMModel(BaseModel):
    ...
    def _create_model(
        ...
        **kwargs,
        ...
        logger.info(
            f"Created new LightGBM model '{model_name}' with parameters: {params}"
        )
    ...
    def _validate_training_data(self, X: pd.DataFrame):
        ...
        if (
            constant_ratio > 0.5
        ):  # If more than 50% of features are constant, this is critical.
            raise ValueError(
                f"CRITICAL: {constant_ratio:.1%} of features are constant. "
            ...
            logger.warning(
                "A significant number of features are constant. Model performance will likely be poor."
            )
        ...
        def objective(trial):
            ...
            params = {
                ...
                "learning_rate": trial.suggest_float(
                    "learning_rate", 0.0001, 0.15, log=True
                ),
                ...
                "min_child_weight": trial.suggest_float(
                    "min_child_weight", 1, 30, step=0.1
                ),
            ...
            try:
                ...
                trial_model = self._create_model(
                    params=params, model_name=f"lightgbm_trial_{trial.number}"
                )
    ...
    def save_model(
        ...
        logger.info(
            f"✅ LightGBM model saved using universal logging function. Run ID: {run_id}"
        )
        return run_id
    ...
    def load_model(self, run_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ LightGBM model loaded successfully from MLflow run: {run_id}"
            )

        except Exception as e:
            logger.error(
                f"❌ Error loading LightGBM model from MLflow run {run_id}: {str(e)}"
            )
            raise
    ...
    def get_prediction_confidence(
        ...
        if method == "leaf_depth":
            leaf_indices = self.model.predict(
                X, pred_leaf=True, num_iteration=self.model.best_iteration
            )
    ...
    def select_features(
        ...
        prelim_params = {
            ...
            "learning_rate": 0.05,
            "verbose": -1,
            "num_threads": self.max_cpu_cores,
            "seed": 42,
        }
    ...
    try:
        ...
        with mlflow.start_run(
            ...
            predictions_example = model.predict(
                input_example, num_iteration=model.best_iteration
            )
...
def main():
    ...
    try:
        ...
        logger.info(
            "✅ Model registered with timestamp-based name via universal function"
        )
