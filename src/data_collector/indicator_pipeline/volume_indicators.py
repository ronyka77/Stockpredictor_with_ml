...
def calculate_obv(data: pd.DataFrame) -> IndicatorResult:
    ...
    try:
        ...
        metadata = {
            ...
            'features': [
                'OBV', 'OBV_SMA_10', 'OBV_SMA_20', 'OBV_Above_SMA_10', 'OBV_Above_SMA_20',
                'OBV_Rising', 'OBV_Falling', 'OBV_Momentum_5', 'OBV_Momentum_10', 'OBV_Price_Divergence'
            ]
        }
...
def calculate_vpt(data: pd.DataFrame) -> IndicatorResult:
    ...
    try:
        ...
        metadata = {
            ...
            'data_points': len(data),
            'features': [
                'VPT', 'VPT_SMA_10', 'VPT_SMA_20', 'VPT_Above_SMA_10', 'VPT_Above_SMA_20',
                'VPT_Rising', 'VPT_Falling', 'VPT_Momentum_5', 'VPT_Momentum_10',
                'VPT_ROC_5', 'VPT_ROC_10'
            ]
        }
...
def calculate_ad_line(data: pd.DataFrame) -> IndicatorResult:
    ...
    try:
        ...
        metadata = {
            ...
            'data_points': len(data),
            'features': [
                'AD_Line', 'AD_SMA_10', 'AD_SMA_20', 'AD_Above_SMA_10', 'AD_Above_SMA_20',
                'AD_Rising', 'AD_Falling', 'AD_Momentum_5', 'AD_Momentum_10',
                'AD_Oscillator', 'AD_Oscillator_Above_Zero'
            ]
        }
...
def calculate_volume_profile(data: pd.DataFrame, periods: Optional[List[int]] = None) -> IndicatorResult:
    """
    ...
    try:
        ...
        for period in periods:
            ...
            volume_trend = data['volume'].rolling(period).apply(
                lambda x: 1 if len(x) >= 2 and not pd.isna(x.iloc[-1]) and not pd.isna(x.iloc[0]) and x.iloc[-1] > x.iloc[0] else 0
            )
        ...
        if len(data) >= 50:
            ...
            result_data['Volume_Percentile_50'] = vol_50.apply(
                lambda x: (x.iloc[-1] - x.min()) / (x.max() - x.min()) if x.max() != x.min() and not pd.isna(x.iloc[-1]) and not pd.isna(x.min()) and not pd.isna(x.max()) else 0.5
            )
        ...
        metadata = {
            ...
            'analysis_types': ['Moving Averages', 'Ratios', 'Trends', 'Price-Volume Correlation', 'Regime Detection']
        }
...
def calculate_money_flow_index(data: pd.DataFrame, period: Optional[int] = None) -> IndicatorResult:
    ...
    try:
        ...
        mfi_values = ta.volume.money_flow_index(data['high'], data['low'], data['close'], data['volume'], window=period)
        ...
        metadata = {
            ...
            'data_points': len(data),
            'features': [
                'MFI', 'MFI_Overbought', 'MFI_Oversold', 'MFI_Neutral',
                'MFI_Rising', 'MFI_Falling', 'MFI_Momentum_5', 'MFI_Momentum_10', 'MFI_Price_Divergence'
            ],
...
class VolumeIndicatorCalculator(BaseIndicator):
    ...
    def calculate(self) -> IndicatorResult:
        ...
        try:
            ...
            # Combine warnings
            all_warnings = (
                obv_result.warnings + vpt_result.warnings + ad_result.warnings +
                volume_profile_result.warnings + mfi_result.warnings
            )
