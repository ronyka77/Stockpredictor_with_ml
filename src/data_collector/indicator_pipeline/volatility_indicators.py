...
def calculate_bollinger_bands(
    data: pd.DataFrame, period: Optional[int] = None, std_dev: Optional[float] = None
) -> IndicatorResult:
    """
    ...
    try:
        ...
        result_data["BB_Position"] = (data["close"] - result_data["BB_Lower"]) / (
...
def calculate_atr(data: pd.DataFrame, period: Optional[int] = None) -> IndicatorResult:
    ...
    try:
        ...
        atr_values = ta.volatility.average_true_range(
...
def calculate_custom_volatility(
    ...
    def calculate(self) -> IndicatorResult:
        ...
        try:
            ...
            combined_data = pd.concat(
                [bb_result.data, atr_result.data, custom_vol_result.data], axis=1
            )
