...
class BaseIndicator(ABC):
    ...
    def validate_data(self) -> None:
        ...
        if len(self.data) < min_points:
            raise ValueError(
                f"Insufficient data points. Need at least {min_points}, got {len(self.data)}"
            )
...
class IndicatorValidator:
    ...
    def validate_result(
        ...
        if result.quality_score < min_quality_score:
            logger.warning(
                f"Validation failed: Quality score {result.quality_score} below threshold {min_quality_score}"
            )
            return False
