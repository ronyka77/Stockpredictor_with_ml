...
class OptimizedFundamentalCollector:
    ...
    async def _process_cached_data(self, ticker: str, cached_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success_count > 0:
                logger.info(f"Successfully stored {success_count} statement periods for {ticker} from cache")
    ...
    def _find_matching_statement(self, statements: List, end_date: str, fiscal_period: str, fiscal_year: str):
        ...
        for stmt in statements:
            ...
            if (stmt_end_date == end_date and 
                stmt_fiscal_period == fiscal_period and 
                stmt_fiscal_year == fiscal_year):
                return stmt
        return None
    ...
    def _prepare_raw_data(self, ticker_id: int, income_stmt: Any, balance_sheet: Any, cash_flow: Any) -> Dict[str, Any]:
        ...
        income_values = self._extract_financial_values(income_stmt, [
            'revenues', 'cost_of_revenue', 'gross_profit', 'operating_expenses', 'net_income_loss'
        ])
        
        balance_values = self._extract_financial_values(balance_sheet, [
            'assets', 'current_assets', 'liabilities', 'equity', 'long_term_debt'
        ]) if balance_sheet else {}
        
        cash_flow_values = self._extract_financial_values(cash_flow, [
            'net_cash_flow_from_operating_activities', 'net_cash_flow_from_investing_activities'
        ]) if cash_flow else {}
        ...
        return {
            ...
            'net_cash_flow_from_operating_activities': cash_flow_values.get('net_cash_flow_from_operating_activities'),
            'net_cash_flow_from_investing_activities': cash_flow_values.get('net_cash_flow_from_investing_activities'),
            ...
            'data_source_confidence': income_values.get('data_source_confidence') # Add data_source_confidence
        }
            ...
            if isinstance(stmt, dict):
                ...
                if 'financials' in stmt:
                    ...
                    for stmt_type in ['income_statement', 'balance_sheet', 'cash_flow_statement']:
                        if stmt_type in financials and field_name in financials[stmt_type]:
                ...
                if isinstance(value, dict):
                    ...
                    if 'value' in value:
                        ...
                        if 'source' in value and value['source']:
                            confidences.append(calculate_source_confidence(value['source']))
                    else:
                        values[field] = value
                else:
                    # Object format (from API)
