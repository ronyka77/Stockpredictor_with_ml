        ...
        try:
            with self.db_pool.get_connection() as conn:
                with conn.cursor() as cursor:
                    # Get total tickers
                    cursor.execute(
                        "SELECT COUNT(*) FROM tickers WHERE active = true and has_financials = true"
                    )
            ...
            with self.db_pool.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    return {
                        ...
                        "field_completeness": {
                            "revenues": field_completeness["revenues_count"]
                            / field_completeness["total_records"]
                            if field_completeness["total_records"] > 0
                            else 0,
                            "net_income": field_completeness["net_income_count"]
                            / field_completeness["total_records"]
                            if field_completeness["total_records"] > 0
                            else 0,
                            "assets": field_completeness["assets_count"]
                            / field_completeness["total_records"]
                            if field_completeness["total_records"] > 0
                            else 0,
                            "cash_flow": field_completeness["cash_flow_count"]
                            / field_completeness["total_records"]
                            if field_completeness["total_records"] > 0
                            else 0,
                        },
