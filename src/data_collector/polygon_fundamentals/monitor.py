...
class FundamentalDataMonitor:
    ...
    def get_collection_progress(self) -> Dict[str, Any]:
        ...
        try:
            total_row = fetch_one("SELECT COUNT(*) as count FROM tickers WHERE active = true and has_financials = true")
            total_tickers = int(total_row.get("count") if total_row else 0)

            tickers_with_data_row = fetch_one("SELECT COUNT(DISTINCT ticker_id) as count FROM raw_fundamental_data")
            tickers_with_data = int(tickers_with_data_row.get("count") if tickers_with_data_row else 0)

            recent_row = fetch_one("SELECT COUNT(DISTINCT ticker_id) as count FROM raw_fundamental_data WHERE date >= CURRENT_DATE - INTERVAL '30 days'")
    ...
    def get_data_quality_summary(self) -> Dict[str, Any]:
        ...
        try:
            avg_row = fetch_one("SELECT AVG(data_quality_score) as avg FROM raw_fundamental_data WHERE data_quality_score IS NOT NULL")
            avg_quality = float(avg_row.get("avg")) if avg_row and avg_row.get("avg") is not None else 0
            ...
            return {
                ...
                "field_completeness": {
                    "revenues": (field_completeness.get("revenues_count", 0) / total_records) if total_records and total_records > 0 else 0,
                    "net_income": (field_completeness.get("net_income_count", 0) / total_records) if total_records and total_records > 0 else 0,
                    "assets": (field_completeness.get("assets_count", 0) / total_records) if total_records and total_records > 0 else 0,
                    "cash_flow": (field_completeness.get("cash_flow_count", 0) / total_records) if total_records and total_records > 0 else 0,
                },
            }
                ...
                recent_activity.append(
                    {
                        ...
                        "filing_date": row["filing_date"].isoformat() if row.get("filing_date") else None,
                        "fiscal_period": row.get("fiscal_period"),
                        "fiscal_year": row.get("fiscal_year"),
                        "data_quality_score": float(row.get("data_quality_score")) if row.get("data_quality_score") else 0,
