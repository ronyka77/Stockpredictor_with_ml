...
class FundamentalDataValidator:
    ...
    def validate_data_completeness(self, response: FundamentalDataResponse, min_periods: int = 4) -> bool:
        """
        Check if we have sufficient data for analysis
        
        Args:
            response: FundamentalDataResponse to check
            min_periods: Minimum number of periods required
        
        Returns:
            True if data is sufficient for analysis
        """
        # Count unique periods across all statements
        periods = set()
        
        for stmt in response.income_statements:
            if stmt.fiscal_period:
                periods.add(f"{stmt.fiscal_year}_{stmt.fiscal_period}")
        ...
        return len(periods) >= min_periods
    
    def calculate_data_quality_metrics(self, response: FundamentalDataResponse) -> Dict[str, Any]:
        """
        Calculate comprehensive data quality metrics
        
        Args:
            response: FundamentalDataResponse to analyze
        ...
        # Count fields across all statements
        for stmt_list in [response.income_statements, response.balance_sheets, response.cash_flow_statements]:
            for stmt in stmt_list:
                stmt_dict = stmt.dict() if hasattr(stmt, 'dict') else vars(stmt)
                for key, value in stmt_dict.items():
                    if key not in ['start_date', 'end_date', 'filing_date', 'period_of_report_date']:
                        total_fields += 1
                        if value is not None:
                            if hasattr(value, 'value') and value.value is not None:
                                populated_fields += 1
                            elif not hasattr(value, 'value'):
                                populated_fields += 1
        ...
        return {
            ...
            'missing_fields_count': len(validation_result.missing_fields),
            'error_count': len(validation_result.errors),
            'warning_count': len(validation_result.warnings),
            'outlier_count': len(validation_result.outliers),
            'statement_counts': {
                'income_statements': len(response.income_statements),
                'balance_sheets': len(response.balance_sheets),
                'cash_flow_statements': len(response.cash_flow_statements)
            },
            'has_sufficient_data': self.validate_data_completeness(response),
            'errors': validation_result.errors,
            'warnings': validation_result.warnings,
            'missing_fields': validation_result.missing_fields,
            'outliers': validation_result.outliers
        } 