...
def get_connection_pool(
    ...
    if _connection_pool is None:
        with _pool_lock:
            ...
            if _connection_pool is None:
                logger.info(
                    f"Initializing fundamental data connection pool ({min_connections}-{max_connections} connections)"
                )
        ...
        with _pool_lock:
            ...
            if current_min != min_connections or current_max != max_connections:
                logger.info(
                    f"Connection pool parameters changed from ({current_min}-{current_max}) to ({min_connections}-{max_connections}). Reinitializing..."
                )
