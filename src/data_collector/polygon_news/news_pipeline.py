...
class PolygonNewsCollector:
    ...
    def collect_historical_news(
        self, max_tickers: int = 50, years_back: int = 2, batch_size_days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Processing {len(major_tickers)} major tickers for historical backfill"
            )
    ...
    def _collect_ticker_news(
        ...
        try:
            ...
            self.logger.info(
                f"Fetching news for {ticker} from {start_date.date()} to {end_date.date()}"
            )
            ...
            for raw_article in raw_articles:
                try:
                    ...
                    # Validate quality
                    is_valid, quality_score, issues = self.validator.validate_article(
                        processed_article
                    )
    ...
    try:
        ...
        try:
            ...
            stats_historical = collector.collect_historical_news(
                max_tickers=3000, years_back=1, batch_size_days=30
            )
