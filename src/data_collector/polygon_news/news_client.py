...
class PolygonNewsClient(PolygonDataClient):
    ...
    def get_news_for_multiple_tickers(self,
                                    tickers: List[str],
                                    published_utc_gte: Optional[datetime] = None,
                                    published_utc_lte: Optional[datetime] = None,
                                    order: str = "desc",
                                    limit_per_ticker: int = 100) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        if failed_tickers:
            logger.warning(f"Failed to fetch news for {len(failed_tickers)} tickers: {failed_tickers}")
        
        total_articles = sum(len(articles) for articles in results.values())
        logger.info(f"Successfully fetched {total_articles} total articles for {len(tickers)} tickers")
    ...
    def get_recent_market_news(self,
        ...
        logger.info(f"Fetching recent market news for {days_back} days across {len(major_tickers)} tickers")
    ...
    def get_news_by_date_range(self,
                                start_date: datetime,
                                end_date: datetime,
    ...
    def extract_article_metadata(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            'sentiment_reasoning': sentiment_insight.get('sentiment_reasoning') if sentiment_insight else None,
            'insights': insights
        }
