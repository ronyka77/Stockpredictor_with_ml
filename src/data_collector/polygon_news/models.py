...
from sqlalchemy import Column, Integer, String, Text, DateTime, Boolean, Float, ForeignKey, ARRAY
...
class PolygonNewsArticle(Base):
    ...
    # Relationships
    tickers = relationship("PolygonNewsTicker", back_populates="article", cascade="all, delete-orphan")
    insights = relationship("PolygonNewsInsight", back_populates="article", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f"<PolygonNewsArticle(id={self.id}, polygon_id='{self.polygon_id}', title='{self.title[:50]}...')>"
    
    @classmethod
    def get_latest_date_for_ticker(cls, session: Session, ticker: str) -> Optional[datetime]:
        """Get the latest published date for articles associated with a ticker"""
        result = session.query(func.max(cls.published_utc)).join(
            PolygonNewsTicker
        ).filter(
    ...
    @classmethod
    def cleanup_old_articles(cls, session: Session, cutoff_date: Optional[datetime] = None) -> int:
        """Remove articles older than cutoff date (default: from config)"""
        if cutoff_date is None:
            from datetime import timezone
            cutoff_date = datetime.now(timezone.utc) - timedelta(days=config.NEWS_RETENTION_YEARS * 365)
    ...
    @classmethod
    def get_article_by_polygon_id(cls, session: Session, polygon_id: str) -> Optional['PolygonNewsArticle']:
...
class PolygonNewsTicker(Base):
    ...
    id = Column(Integer, primary_key=True)
    article_id = Column(Integer, ForeignKey('polygon_news_articles.id', ondelete='CASCADE'), nullable=False)
    ...
    def __repr__(self):
        return f"<PolygonNewsTicker(article_id={self.article_id}, ticker='{self.ticker}')>"
    
    @classmethod
    def get_articles_for_ticker(cls, session: Session, ticker: str, 
                                start_date: Optional[datetime] = None,
                                end_date: Optional[datetime] = None) -> List[PolygonNewsArticle]:
        """Get all articles associated with a ticker within date range"""
        query = session.query(PolygonNewsArticle).join(cls).filter(
    ...
    @classmethod
    def get_ticker_counts(cls, session: Session, 
                            start_date: Optional[datetime] = None,
...
class PolygonNewsInsight(Base):
    ...
    @classmethod
    def get_sentiment_distribution(cls, session: Session,
                                    start_date: Optional[datetime] = None,
                                    end_date: Optional[datetime] = None) -> Dict[str, int]:
        """Get distribution of sentiment across articles"""
        query = session.query(cls.sentiment, func.count(cls.id)).filter(
    ...
    @classmethod
    def get_sentiment_for_ticker(cls, session: Session, ticker: str,
                                start_date: Optional[datetime] = None,
                                end_date: Optional[datetime] = None) -> List['PolygonNewsInsight']:
        """Get sentiment insights for a specific ticker"""
        query = session.query(cls).join(PolygonNewsArticle).join(PolygonNewsTicker).filter(
            PolygonNewsTicker.ticker == ticker.upper(),
            cls.insight_type == 'sentiment',
            cls.sentiment.isnot(None)
        )
