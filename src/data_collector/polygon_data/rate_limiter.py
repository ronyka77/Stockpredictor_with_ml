...
class RateLimiter:
    ...
    def wait_if_needed(self) -> None:
        ...
        if self.request_count >= self.requests_per_minute:
            ...
            if sleep_time > 0:
                logger.info(f"Rate limit reached ({self.request_count}/{self.requests_per_minute}). "
    ...
    def __str__(self) -> str:
        ...
        return (f"RateLimiter(requests: {self.request_count}/{self.requests_per_minute}, "
...
class AdaptiveRateLimiter(RateLimiter):
    ...
    def handle_rate_limit_error(self) -> None:
        ...
        logger.warning(f"Rate limit error detected. Reducing limit from {old_limit} to "
    ...
    def handle_successful_request(self) -> None:
        ...
        if self.consecutive_errors > 0:
            ...
            if self.consecutive_errors == 0 and self.requests_per_minute < self.original_limit:
                old_limit = self.requests_per_minute
                self.requests_per_minute = min(self.original_limit, 
                                            int(self.requests_per_minute / self.backoff_factor))
                
                logger.info(f"Restoring rate limit from {old_limit} to "
                            f"{self.requests_per_minute} requests/minute")
