...
class ArchitectureAnalyzer:
    ...
    def _identify_repositories(self) -> List[str]:
        ...
        for module_name, module_info in self.modules.items():
            ...
            elif any('storage' in cls.lower() or 'repository' in cls.lower()
    ...
    def _check_missing_abstractions(self, issues: List[DependencyIssue]) -> None:
        ...
        for module_name, imports in import_clusters.items():
            direct_data_imports = [imp for imp in imports
    ...
    def generate_recommendations(self, issues: List[DependencyIssue]) -> List[str]:
        ...
        if not issues:
            recommendations.append("Architecture appears well-structured with no major issues detected")
        ...
        if 'circular_import' in issues_by_category:
            recommendations.append(f"Fix {len(issues_by_category['circular_import'])} circular import issues to improve maintainability")

        if 'tight_coupling' in issues_by_category:
            recommendations.append("Reduce tight coupling by breaking down large modules into smaller, focused components")

        if 'missing_abstraction' in issues_by_category:
            recommendations.append("Introduce abstraction layers to reduce direct dependencies on data sources")
...
def assess_data_collector_architecture(root_path: Path) -> ArchitectureAssessment:
    ...
    logger.info(f"Architecture assessment complete. Found {len(issues)} issues, {len(patterns)} patterns")
        ...
        def run(self):
            ...
            try:
                ...
                if not target_path.exists():
                    return 0.0, [{"type": "error", "message": f"Target directory {target_path} does not exist"}], \
                ...
                score = 1.0 - (len(assessment.issues) / max(len(assessment.modules), 1))  # Higher score = fewer issues
                ...
                self.logger.info(f"Architecture assessment completed. Score: {score:.2f}, {len(findings)} findings")

                return score, findings, recommendations

            except Exception as e:
                self.logger.error(f"Architecture assessment failed: {e}")
                return 0.0, [{"type": "error", "message": f"Assessment failed: {str(e)}"}], \
