...
def prepare_ml_data_for_training(prediction_horizon: int = 10, 
    ...
    Args:
        ...
        apply_stationarity_transform: Whether to apply stationarity transformations (default: False)
    ...
    try:
        ...
        exclude_cols = [
            ...
            'feature_categories', 'file_path', 'warnings', 
            'quality_score', 'record_count', 'total_features', 'file_size_mb'
        ]
...
def prepare_ml_data_for_prediction(prediction_horizon: int = 10) -> Dict[str, Union[pd.DataFrame, pd.Series, str]]:
    ...
    try:
        ...
        if not test_dates_for_pred.empty:
            logger.info("📅 Filtering prediction set to include only Mondays and Fridays.")
            is_monday_or_friday_pred = (test_dates_for_pred.dt.dayofweek == 0) | (test_dates_for_pred.dt.dayofweek == 4)
...
def prepare_ml_data_for_training_with_cleaning(prediction_horizon: int = 10,
    ...
    """
    logger.info(f"📊 [START] Preparing ML data with cleaning (horizon: {prediction_horizon}d, split: {split_date})")
...
def prepare_ml_data_for_prediction_with_cleaning(prediction_horizon: int = 10,
    ...
    """
    logger.info(f"📊 Preparing prediction data with cleaning (horizon: {prediction_horizon}d, days_back: {days_back})")
