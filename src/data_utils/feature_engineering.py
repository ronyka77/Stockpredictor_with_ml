...
def clean_features_for_training(X: pd.DataFrame, y: pd.Series, 
    ...
    logger.info(f"✅ Feature cleaning completed: {X_clean.shape[0]} samples, {X_clean.shape[1]} features")
...
def validate_feature_quality(features: pd.DataFrame,
                            ...
                            min_variance_threshold: float = 1e-8) -> dict:
    """
    Validate the quality of feature data for ML training
    ...
    """
    logger.info(f"🔍 Validating feature quality for '{feature_name}'...")
    ...
    numeric_count = len(numeric_features.columns)
    
    validation_results['stats'] = {
        'total_features': total_features,
        'numeric_features': numeric_count,
        'non_numeric_features': total_features - numeric_count,
    ...
    if total_features == 0:
        ...
        validation_results['issues'].append("No features found")
        validation_results['recommendations'].append("Check data loading process")
        return validation_results
    
    # Check 2: Missing values analysis
    missing_analysis = features.isnull().sum()
    high_missing_features = missing_analysis[missing_analysis / len(features) * 100 > max_missing_pct]
    
    if not high_missing_features.empty:
        validation_results['issues'].append(f"{len(high_missing_features)} features have >{max_missing_pct}% missing values")
        validation_results['recommendations'].append("Consider removing or imputing high-missing features")
        if len(high_missing_features) > total_features * 0.5:  # More than 50% of features
            validation_results['is_valid'] = False
    
    # Check 3: Variance analysis
    if not numeric_features.empty:
        variances = numeric_features.var()
        low_variance_count = (variances < min_variance_threshold).sum()
        zero_variance_count = (variances == 0).sum()
        
        validation_results['stats']['low_variance_features'] = low_variance_count
        validation_results['stats']['zero_variance_features'] = zero_variance_count
        
        if zero_variance_count > 0:
            validation_results['issues'].append(f"{zero_variance_count} features have zero variance")
            validation_results['recommendations'].append("Remove constant features")
        
        if low_variance_count > numeric_count * 0.3:  # More than 30% low variance
            validation_results['issues'].append(f"{low_variance_count} features have very low variance")
            validation_results['recommendations'].append("Consider feature selection or transformation")
    
    # Check 4: Data types
    object_columns = features.select_dtypes(include=['object']).columns
    if len(object_columns) > 0:
        validation_results['issues'].append(f"{len(object_columns)} features are non-numeric")
        validation_results['recommendations'].append("Convert categorical features to numeric or remove them")
    
    # Check 5: Infinite values
    if not numeric_features.empty:
        inf_counts = np.isinf(numeric_features).sum().sum()
        if inf_counts > 0:
            validation_results['issues'].append(f"{inf_counts} infinite values found")
            validation_results['recommendations'].append("Handle infinite values before training")
    
    # Log validation results
    if validation_results['is_valid']:
        logger.info(f"✅ Feature validation passed for '{feature_name}'")
        logger.info(f"   Total features: {total_features}")
        logger.info(f"   Numeric features: {numeric_count}")
        logger.info(f"   Samples: {len(features):,}")
    else:
        logger.warning(f"⚠️ Feature validation failed for '{feature_name}'")
