...
class DatabaseConnection:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Initialized DatabaseConnection for {self.config['host']}:{self.config['port']}/{self.config['database']}"
        )
...
class DatabaseConnectionPool:
    ...
    def __init__(
        ...
        if not self.config.get("password"):
            raise ValueError(
                "DB_PASSWORD environment variable is required for database connection pooling"
            )
        ...
        logger.info(
            f"Initialized DatabaseConnectionPool ({min_connections}-{max_connections} connections) "
