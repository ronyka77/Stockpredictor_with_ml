...
def test_identify_categorical_features_includes_ticker_and_low_cardinality(sample_tabular_data, lgb_model_instance):
...
def test_predict_warns_on_feature_mismatch_and_returns_array(lgb_model_instance, sample_tabular_data):
...
def test_log_model_to_mlflow_calls_helper(lgb_model_instance, sample_tabular_data):
    ...
    with patch("src.models.gradient_boosting.lightgbm_model.log_to_mlflow_lightgbm", return_value="run-123") as mock_log:

        # Execution
        # Ensure a model object exists to satisfy precondition
        lgb_model_instance.model = Mock()
        run_id = lgb_model_instance.log_model_to_mlflow(metrics=metrics, params=params, X_eval=X_eval, experiment_name="exp")
...
def test_load_model_uses_mlflow_client_and_handles_missing_signature(lgb_model_instance):
    ...
    with patch("src.models.gradient_boosting.lightgbm_model.mlflow.tracking.MlflowClient", return_value=fake_mlflow_client), \
         patch("src.models.gradient_boosting.lightgbm_model.mlflow.lightgbm.load_model", return_value=Mock()):
