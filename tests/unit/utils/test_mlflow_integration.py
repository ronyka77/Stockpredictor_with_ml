...
def test_setup_experiment_creates_or_uses_existing(existing_experiment, patch_mlflow_start):
    ...
    with patch("mlflow.get_experiment_by_name", new=fake_get), patch(
        "mlflow.create_experiment", new=Mock(return_value="created-id")
...
def test_setup_experiment_with_artifact_location_calls_create():
    ...
    with patch("mlflow.get_experiment_by_name", new=fake_get), patch(
        "mlflow.create_experiment", new=fake_create
    ), patch("mlflow.set_experiment", new=Mock()):
        mlfi = MLflowIntegration()
...
def test_log_params_and_metrics_use_active_or_run():
    with patch("mlflow.log_params", new=Mock()) as fake_log_params, patch(
        "mlflow.log_metrics", new=Mock()
    ) as fake_log_metrics:
        mlfi = MLflowIntegration()
        mlfi.log_params({"p": 1})
...
def test_log_and_load_model_flow():
    ...
    with patch("mlflow.sklearn.log_model", new=Mock()) as fake_log_model, patch(
        "mlflow.pyfunc.load_model", new=Mock(return_value=fake_model)
    ):
        mlfi = MLflowIntegration()
...
def test_cleanup_deleted_runs_handles_rmtree_exception(tmp_mlruns_dir, patch_mlflow_client):
    ...
    with patch("os.path.exists", return_value=True), patch("shutil.rmtree", side_effect=OSError("boom")):
        cleanup_deleted_runs(str(mlruns_dir))
