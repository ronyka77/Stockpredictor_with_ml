...
@pytest.mark.unit
def test_store_article_on_exception_rolls_back_and_returns_none(
    db_session, sample_raw_article_full
):
    storage = PolygonNewsStorage(db_session)

    # Ensure no existing article with same polygon_id to force create path
    try:
        db_session.query(PolygonNewsArticle).filter_by(
            polygon_id=sample_raw_article_full["polygon_id"]
        ).delete()
        db_session.commit()
    except Exception:
