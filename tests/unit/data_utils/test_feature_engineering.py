...
def test_add_price_normalized_features_creates_sma_ratio_and_close_open_ratio():
    ...
    # numeric equality
    assert out["SMA_5_Ratio"].iloc[0] == pytest.approx(100.0 / 95.0), "SMA ratio incorrect"
    assert out["Close_Open_Ratio"].iloc[1] == pytest.approx(110.0 / 109.0), "Close/Open ratio incorrect"
...
def test_add_prediction_bounds_features_populates_expected_context_columns():
    ...
    assert out["Expected_10D_Move"].iloc[0] == pytest.approx(0.02 * np.sqrt(10)), "Expected_10D_Move incorrect"
...
def test_clean_data_for_training_handles_inf_extreme_and_nan():
    ...
    df = pd.DataFrame({
        ...
        "c": ["x", "y", "z", None]  # non-numeric should be preserved but coerced only for numeric cols
    })

    out = fe.clean_data_for_training(df)
    # numeric columns become float64 and have no NaN left
    numeric_cols = out.select_dtypes(include=[np.number]).columns.tolist()
    assert "a" in numeric_cols and "b" in numeric_cols, f"Missing numeric columns: {numeric_cols}"
...
def test_clean_features_for_training_removes_non_numeric_and_high_corr_preserves_essential():
    ...
    assert "const" not in X_clean.columns or "const" in removed["constant"], "Constant column removal mismatch"
