...
def test_add_price_normalized_features_schema_and_values():
    ...
    assert list(out.columns) == expected_cols, f"Unexpected schema after add_price_normalized_features: got {list(out.columns)}, expected {expected_cols}"

    assert out['SMA_5_Ratio'].dtype == np.float64, f"Unexpected dtype for SMA_5_Ratio: {out['SMA_5_Ratio'].dtype}"
    assert out['Close_Open_Ratio'].dtype == np.float64, f"Unexpected dtype for Close_Open_Ratio: {out['Close_Open_Ratio'].dtype}"

    pdt.assert_series_equal(out['SMA_5_Ratio'].reset_index(drop=True), pd.Series([100.0/95.0, 110.0/105.0], dtype='float64'), check_names=False)
...
def test_add_prediction_bounds_features_expected_columns_and_values():
    ...
    expected_new = ['Expected_Daily_Move', 'Expected_10D_Move', 'Recent_Momentum_5D', 'Momentum_Acceleration', 'RSI_Mean_Reversion_Pressure', 'BB_Range_Context']
    for col in expected_new:
        assert col in out.columns, f"Expected column {col} to be added by add_prediction_bounds_features"

    assert out['Expected_10D_Move'].iloc[0] == pytest.approx(0.02 * np.sqrt(10)), "Expected_10D_Move computation incorrect"
...
def test_clean_data_for_training_handles_inf_extreme_and_nan_and_dtypes():
    ...
    df = pd.DataFrame({
        ...
        'c': pd.Series(['x', 'y', 'z', None])  # non-numeric should be preserved but coerced only for numeric cols
    })

    out = clean_data_for_training(df.copy())

    numeric_cols = out.select_dtypes(include=[np.number]).columns.tolist()
    assert 'a' in numeric_cols and 'b' in numeric_cols, f"Missing numeric columns after cleaning: {numeric_cols}"

    assert not out[numeric_cols].isnull().any().any(), f"NaNs remain after cleaning: {out[numeric_cols].isnull().sum().to_dict()}"
...
def test_add_temporal_features_creates_expected_columns():
    ...
    assert out.loc[0, 'year'] == 2025 and out.loc[1, 'year'] == 2025, "Year extraction failed"
